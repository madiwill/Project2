...
def get_umsi_data():
    ...
    for page_num in range(0,13):
        html = requests.get("https://www.si.umich.edu/directory?field_person_firstname_value&amp;field_person_lastna&page="+str(page_num), headers={'User-Agent': 'SI_CLASS'})
        page = html.text
        soup = BeautifulSoup(page, "html.parser")
        for person in ((soup.find("div", class_="view-content").find_all("div", class_="views-row"))):
            name = person.find("div", class_="field-name-title")
            title = person.find("div", class_="field-name-field-person-titles")
            dic[name.text] = title.text
    return (dic)
